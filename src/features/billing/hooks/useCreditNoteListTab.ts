...
export function useCreditNoteListTab({
  ...
  const ArchiveInvoice = async () => {
    try {
      ...
      if (selectedCreditNote) {
        const formData = new FormData();
        formData.append("status", invoiceStatusSchema.enum.ARCHIVED);
        await InvoicesCreditNoteAPI.updateInvoiceCreditNoteStatus(selectedCreditNote?.invoiceCreditNoteNumber, formData);
        appToast.success('Statut mise à jour avec succès.')
      ...
      appToast.error("Erreur dans la mise à jour:", getApiErrorMessage(error));
    }
    finally {
  ...
  return {
    ...
    deleteLoading,
    deleteId,
    setDeleteId,fetchCreditNotes,
    loadingArchive, setLoadingArchive,
    archiveOpen, setArchiveOpen,ArchiveInvoice,
