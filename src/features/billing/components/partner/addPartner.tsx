...
export default function AddPartnerPage({type} : pageProps) {
  const {
    copyBillingToShipping ,documentFields,isClient,router,
    onSubmit,getError,register,handleSubmit,setValue,setValueAny,isSubmitting
    ,watch
    } = UseCreatePartner({ type })
  return (
      ...
      <header className="bg-white border-b border-gray-100 px-8 py-6">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-black text-gray-900 tracking-tighter">Nouveau {type === partnerTypeSchema.enum.CLIENT ? "Client" : "Fournisseur"}</h1>
              <p className="text-sm font-bold text-gray-600 mt-1">Créer un nouveau {type === partnerTypeSchema.enum.CLIENT ? "Client" : "Fournisseur"}</p>
            </div>
          </div>
        </div>
      </header>
